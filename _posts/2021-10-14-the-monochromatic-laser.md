...
blah blah femtometer stability

To control the ECDL in the configuration proposed above, three drivers are required: a current driver for the laser diode, a temperature controller for the Peltier device and temperature sensor, and a voltage controller for the piezo actuator. In this work, we do not cover such drivers in detail, as there are countless valid circuits for these purposes that are well-studied.

For the purposes of our experiment, we utilize the works described in \cite{minho}, \cite{current}, and \cite{piezo}. One practical motivation for the usage of these drivers is that they are all digitally-enabled, which allows us to have end-to-end data collection and feedback on the same processor. In particular, a Teensy 3.5 digital microcontroller is used to manage the three component drivers. Figure \ref{fig:temppcb} depicts a PCB diagram for the temperature controller used in our experiment.

### Temperature Control

Another key aspect to proper ECDL operation is precise control of the temperature of the laser diode \cite{tunable}. The wavelength of light that is output by a laser diode changes with respect to temperature (see \ref{fig:tempvswl}, typically because the thermal fluctuation will change the length of the internal cavity of the diode. As more current is provided to the laser diode, the temperature will rise, thereby changing the wavelength and possibly disrupting the experiment.
...
\begin{figure}[H]
\centering
\includegraphics[width=3in]{figures/Temp vs Wavelength.png}
\caption{The variation of laser diode wavelength with respect to cavity temperature}
\label{fig:tempvswl}
\end{figure}

To keep the temperature stable about a set value, experimentalists often use analog circuits, in which temperature measurements are taken by an analog sensor (e.g. Analog Devices AD590, \$15), and used in conjunction with a proportional–integral–derivative loop to adjust the input current to the Peltier device such that temperature remains constant. See Figure \ref{fig:diodemount} for an example of such a configuration: the bottom plate depicted is a heatsink; the Peltier device is sandwiched between the heatsink and the bottom half of the diode mount, and a temperature sensor is inserted into the top half of the diode mount (as close to the diode as possible) to get temperature readings.

FIGURE: TEC PCB

### Fine-Tuning Frequency

piezo control, stability, etc

### Optical Isolator

As discussed in Section \ref{sec:ecdloptics}, the mechanics of an ECDL necessarily depend on the nature of stimulated emission; specifically, the fact that light reflected back into the diode can stimulate a higher level of emission at a desired wavelength. However, this effect can also backfire, if light reflected back into the diode is not of the desired wavelength. Without proper precautions, this phenomenon can easily occur in experiments, as the light emitted by the ECDL is reflected by other optical elements in the experiment.

The typical solution to this problem is to utilize an \textbf{optical isolator} in conjunction with the light output from the ECDL. An optical isolator is an optical element that makes use of the physical phenomenon of \textit{Faraday rotation}, or the rotation of the polarization of light within certain nonlinear crystals \cite{fischer_1987}.

\begin{figure}[H]
\centering
\includegraphics[width=5.5in]{figures/isolator.png}
\caption{Typical configuration of an optical isolator}
\label{fig:isolator}
\end{figure}

Such crystals are characterized by their \textit{Verdet constant}, which is a measure of how many radians the polarization of incident light is rotated by, per meter, per Tesla. A Faraday rotator is built by placing a nonlinear crystal within a region of axial magnetic flux; as the light passes through this region, its polarization will be rotated by an angle given by:

$$\Delta \theta = V \beta L,$$

where $V$ is the Verdet constant of the crystal, $\beta$ is the axial magnitude flux in the crystal, and $L$ is the length of the crystal \cite{moller}. To create an optical isolator, then, we first place a linear polarizer (e.g. Melles Griot 03FPG009, \$35) in front of such a Faraday rotator. This ensures that the light entering the Faraday rotator is uniformly polarized. Subsequently, we add a 45° polarizer (a.k.a. analyzer) after the rotator; this ensures that only the light whose polarization has been rotated by 45° can exit the isolator.
...
When we utilize the $780$nm transition of Rubidium-85 to excite atoms in the $5^2 S_{1/2}$ ground state to the $5^2 P_{3/2}$ state, this corresponds to a $F=3 \to F'=4$ transition. By angular selection rules, the $F'=4$ state must decay back into the $F=3$ state, meaning that the atoms can be immediately excited once again (and therefore cooled, via the principles of the radiation pressure force). However, due to the phenomenon of \textit{Raman scattering}, there is a possibility that Rubidium-85 atoms will end up in a hyperfine split $F=2$ state, which is `dark' with respect to the $780$nm light \cite{rbdata}.

More generally, all alkali elements are subject to such a phenomenon in one form or another, which typically necessitates the usage of a secondary \textbf{repumping laser} that removes atoms from this `dark state'. In Rb-85, this involves an excitation from the $F=2$ to the $5^2 P_{3/2}, F'=3$ state, which will then decay back down to $5^2 S_{1/2}, F=3$, the ground state, making such an atom ready for cooling cycles once again. One favorable property of this particular`repumping' transition in Rb-85 is that it is very close in natural frequency to the cooling transition, being separated by only 2.9GHz. This may be compared to another alkali element, such as Potassium-40, which has a cooling transition at $405$nm and a corresponding repumping transition at $767$nm, which is a much wider gap \cite{PhysRevA.84.063420}.

\begin{figure}[H]
\centering
\includegraphics[width=3.2in]{figures/rb repumping.png}
\caption{An energy diagram depicting the cooling and repumping transition for Rubidium-85}
\label{fig:my_label}
\end{figure}

We may take advantage of this property by generating a \textit{sideband} in the output beam $780$nm `cooling' ECDL that can access this Rb-85 repumping transition. In electromagnetic radiation, a sideband is a band of frequencies that is higher or lower than the main`carrier' frequency, which arises as a result of a modulation process (e.g. voltage controlled oscillation or acousto-optical deflection). In other words, we can modulate the $780$nm diode laser such that it will simultaneously generate light at the repumping frequency, so that the experiment can be run with a single ECDL, which greatly reduces both experiment cost and complexity.

## Voltage Controlled Oscillators

A cheap and simple implementation of such modulation can be done via the use of a \textbf{voltage controlled oscillator}, or VCO (e.g. Avantek VTO-8240, \$55). In particular, for Rb-85 repumping, a microwave oscillator whose range of oscillation is approximately centered about 2.9GHz is optimal. Due to the relatively broad ($\approx 100$ MHz) line width of the repumping transition, specific details such as harmonic vs. relaxation oscillation and noise figure are not expressly important.

The VCO is a circuit element that can be placed in parallel with the ECDL circuit and a single additional capacitor. It is provided with a separate DC driving voltage, which controls the frequency of oscillation of the VCO. This, in turn, adds an AC voltage source to the circuit that drives the laser diode. The input current to a laser diode directly affects its cavity length, which means that this AC modulation will cause a corresponding oscillation in the output frequency of the diode (as the diode's cavity length directly controls the output frequency of light) \cite{microwave}.

Since the line width of the repumping transition of Rb-85 is relatively large, tuning the DC input to the VCO can be performed (once the remainder of the experiment is correctly set up) by scanning through a range of input voltages until the steady-state MOT is established.
...
\subsubsection{Atomic Vapor Cells}

## Fabry-Perot Interferometry

A \textbf{Fabry-Perot cavity} (or interferometer) is an optical device constructed with two mirrors in a variable-length tube. When operated, light is transmitted through one end of the cavity, and then is reflected back and forth between the two mirrors inside the cavity. The amount of light that is transmitted through the other end of the cavity is dependent on the length of the cavity, due to the variation in the interference patterns of the reflected light inside the cavity with respect to the length \cite{hernandez_1988}.
...
\begin{figure}[H]
\centering
\includegraphics[width=3in]{figures/fabryperot.png}
\caption{Dynamics of reflection and transmission in a Fabry-Perot cavity}
\label{fig:my_label}
\end{figure}
...
A reliable source of frequency measurement for ground state transitions of atoms is through the usage of \textbf{atomic vapor cells}, which are containers that heat a pure sample of a particular element until it forms a vapor. In an optical experiment, some of the light from the light source can be split off via a beam splitter, directed through such a vapor cell, and then into a photodetector.
...
\begin{figure}[H]
\centering
\includegraphics[width=6in]{figures/vaporcell.png}
\caption{A diagram depicting the usage of an atomic vapor cell (a.k.a. reference cell}
\label{fig:my_label}
\end{figure}
...
Note that atomic vapor cells can only be used as an error signal for ground state transitions, because the vapor cell will only contain atoms in their ground state, and therefore the absorption measurement will only have meaningful fluctuations for transitions that excite atoms from their ground state.
